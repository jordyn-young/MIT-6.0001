...
def get_word_score(word, n):
    ...
    wordlen = len(word)
    given_equation = 7*wordlen - 3*(n-wordlen)
    if given_equation > 1:
        second_component = given_equation
    else:
        second_component = 1
...
def is_valid_word(word, hand, word_list):
    ...
    if word in word_list:
        word = list(word)
        for letter in word:
            if letter in remaining_hand.keys():
                remaining_hand[letter] -= 1
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    if calculate_handlen(hand) == 0 and done_playing == False:
    ...
    for i in range(number_hands):
        if i != 0 and replay_hand != 'yes':
            ...
            if replay_hand == 'yes':
                ...
                print("That is not a valid response, so you will replay your last hand.")
        if replay_hand == 'no' and substitute != 'yes':
            display_hand(hand)
            ...
            if substitute == 'yes':
                ...
                substitute = 'yes'
                print("That is not a valid response, so you will not substitute a letter")
